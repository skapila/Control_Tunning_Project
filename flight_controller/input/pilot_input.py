...
class PilotInput:
    ...
    def _normalize(self, pwm):
        return (pwm - 1500) / 500.0   # -1 to 1,mapping from 1000 to 2000

    def _normalize_throttle(self, pwm):
        return (pwm - self.min_pwm) / (self.max_pwm - self.min_pwm)     # for throttle it is 0 to 1

    def get_desired_roll_angle(self):
        return self._normalize(self.roll_pwm) * self.max_angle   # (-1 to 1)*max angle
    ...
    # Function to scale joystick input to PWM values                                          ### mapping joystick to esc range
    def _scale_joystick(self,value, min_out=1000, max_out=2000):
        return int((value + 1) / 2 * (max_out - min_out) + min_out)
        
    def _apply_deadband(self, pwm, center=1500, threshold=15):
        return pwm if abs(pwm - center) > threshold else center
